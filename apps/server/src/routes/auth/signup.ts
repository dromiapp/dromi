...
import { error, StatusMap, t } from "elysia";
import { prisma } from "@repo/db";
import { Algorithm, hash } from "@node-rs/argon2";
import { createSession, generateSessionToken, hashPassword, nanoid } from "~/src/lib/auth";
    ...
    async (context) => {
      ...
      const new_user = await prisma.user.create({
        data: {
          ...
          username: context.body.username || context.body.email.split("@")[0] as string,
        }
      })
